...
/**
 * Model implementation used by the app.
...
export class LAppModel extends CubismUserModel {
  ...
  public loadAssets(dir: string, fileName: string): void {
    ...
    fetch(`${this._modelHomeDir}${fileName}`)
      ...
      .catch(error => {
        // If model3.json fails to load, drawing is impossible; log and return.
        CubismLogError(`Failed to load file ${this._modelHomeDir}${fileName}`);
      });
  }

  /**
   * Build model from model3.json setting and load components.
   */
          ...
          fetch(`${this._modelHomeDir}${expressionFileName}`)
            .then(response => {
              if (response.ok) {
                ...
                // Use empty ArrayBuffer to handle missing file without null.
                return new ArrayBuffer(0);
              }
            })
            .then(arrayBuffer => {
  ...
  private setupTextures(): void {
    ...
    if (this._state == LoadStep.LoadTexture) {
      ...
      for (
        ...
        // Skip if texture file name is empty
        if (this._modelSetting.getTextureFileName(modelTextureNumber) == '') {
          console.log('getTextureFileName null');
          continue;
        }

        // Load into WebGL texture unit
        let texturePath =
  ...
  public update(): void {
    ...
    // ドラッグによる変化
    // ドラッグによる顔の向きの調整
    this._model.addParameterValueById(this._idParamAngleX, this._dragX * 38); // -38から38の値を加える (reduced horizontal to prevent exceeding center)
    this._model.addParameterValueById(this._idParamAngleY, this._dragY * 50);
    this._model.addParameterValueById(
      this._idParamAngleZ,
      this._dragX * this._dragY * -38
    );
    ...
    this._model.addParameterValueById(
      ...
      this._dragX * 14
    ); // -14から14の値を加える (reduced horizontal to prevent exceeding center)

    // ドラッグによる目の向きの調整
    this._model.addParameterValueById(this._idParamEyeBallX, this._dragX * 1.2); // -1.2から1.2の値を加える (reduced horizontal to prevent exceeding center)
